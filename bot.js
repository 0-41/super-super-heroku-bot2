...
const otaku = [
     ...
     ' ما هوا إلهاكي؟',
     'من هو العضو المهتم بدراسه الآثار من طاقم قبعه القش؟',
     'من هو والابن الحقيقي للحيه البيضاء؟',
     'كم عدد الفواكه التي لد الحيه السوداء وماهيه؟',
     'كم القيمه التي دفعتها البحريه مقابل رأس لوفي؟',
     ' ما نوع القوه التي تتمناها من انمي ون بنش مان؟',
     'كم القيمه التي دفعتها البحريه مقابل رأس لوفي؟',
     'ما هو الانمي بنسبه لك؟',
     'هل تفكر بصناعة انمي ولو بتصنع وش بيكون؟',
     'بماذا تشعر عندما تشاهد انمي؟',
     ' كم عدد الانميات التي تابعتها إلى الآن؟',
     'ما هو وحش البيجو الذي ختم في ناروتو ؟',
     'من انمي ون بيس ، ما نوع فاكهة تريبول؟',
     'من انمي ون بيس ، من اول شخص هرب من سجن الامبل داون؟',
     'من انمي هنتر اكس هنتر ، ما فئة هيسوكا بالنين؟',
     'من انمي هنتر اكس هنتر ، ما اسم تقنية البرق عند كيلوا؟',
     ' من انمي هجوم العمالقة ، ما هو اسم  العملاقين الي قبضت عليهم هانجي؟',
     'من انمي هجوم العمالقة ،  ما هي اصعب شخصية يرسمها الكاتب؟',
     'من انمي هجوم العمالقة كم يبلغ عدد حراس المملكيين؟',
     'من الذي قتل جيرايا في انمي ناروتو؟',
   ,
]
   x5bz.on('message', message => {
...
x5bz.on('message', msg => {
    ...
    if(msg.content ==='avatar'){ 
        const embed = new Discord.RichEmbed()
    .setTitle(`This is [ ${msg.author.username} ] Avatar`)
...
x5bz.on("message", message => {
  ...
  let modlog = x5bz.channels.find('name', 'mute-log');
...
if(!message.guild.member(x5bz.user).hasPermission("MANAGE_ROLES")) return message.reply("البوت لايملك صلاحيات ").then(msg => msg.delete(5000));;
let user = message.mentions.users.first();
if (message.mentions.users.size < 1) return message.reply('**ضع منشن الشخص!!**').then(msg => {msg.delete(5000)});
let MRole = message.content.split(" ").slice(2).join(" ");
if(!MRole)return message.reply("support").then(msg => {msg.delete(5000)});
